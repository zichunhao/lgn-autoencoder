...
def get_node_dev(transform_input, transform_output, eps=1e-16, mode="mean"):
    if mode.lower() == "max":
        return {
            weight: (
                (transform_input[weight] - transform_output[weight])
                / (transform_output[weight] + eps)
            )
            .abs()
            .max()
            .item()
        ...
        return {
            weight: abs(
                (transform_input[weight] - transform_output[weight]).mean().item()
                / ((transform_output[weight]).mean().item() + eps)
            )
            for weight in [(0, 0), (1, 1)]
        }
    else:
...
def get_dev(
    transform_input,
    transform_output,
    transform_input_nodes_all,
    transform_output_nodes_all,
    mode="mean",
):
    # Output equivariance
    dev_output = [
        get_node_dev(transform_input[i], transform_output[i], mode=mode)
    ...
    dev_internal = [
        [
            get_node_dev(
                transform_input_nodes_all[i][j],
                transform_output_nodes_all[i][j],
                mode=mode,
            )
...
def get_avg_output_dev(covariance_results, test_name):
    ...
    output_devs = [
        covariance_results[i][f"{test_name.lower()}_dev_output"]
        for i in range(len(covariance_results))
    ]
    output_devs_by_alpha = np.array(output_devs).transpose().tolist()

    # Adapted from https://stackoverflow.com/a/50782438
    avg_output_dev_by_alpha = [
        {
            key: sum(dev[key] for dev in output_devs_by_alpha[i])
            / len(output_devs_by_alpha[i])
...
def get_avg_internal_dev(covariance_results, test_name):
    ...
    internal_devs = [
        covariance_results[i][f"{test_name.lower()}_dev_internal"]
        for i in range(len(covariance_results))
    ]
    ...
    avg_internal_dev_by_alpha = [
        [
            {
                key: sum(e[key] for e in internal_devs_by_alpha[i][j])
                / len(internal_devs_by_alpha[i][j])
...
def get_internal_dev_stats(dev_internal):
    ...
    for i in range(len(dev_internal)):
        ...
        dev_internal_mean.append(
            {
                (0, 0): sum(boost_dev_level_scalar) / len(boost_dev_level_scalar),
                (1, 1): sum(boost_dev_level_p4) / len(boost_dev_level_p4),
            }
        )
        dev_internal_max.append(
            {(0, 0): max(boost_dev_level_scalar), (1, 1): max(boost_dev_level_p4)}
        )

    dev_internal_mean = {
        key: [dev_internal_mean[i][key] for i in range(len(dev_internal_mean))]
        for key in [(0, 0), (1, 1)]
    }
    dev_internal_max = {
        key: [dev_internal_max[i][key] for i in range(len(dev_internal_max))]
        for key in [(0, 0), (1, 1)]
    }
...
def plot_internal_dev(
    dev_internal, alphas, transform_type, weight, save_path, show_all=False
):
    """
    ...
    dev_internal_mean, dev_internal_max, dev_layers = get_internal_dev_stats(
        dev_internal
    )
    ...
    if transform_type.lower() in ["boost", "boosts"]:
        ...
        plt.title(
            f"Rotation equivariance test of internal {irrep_str} features", y=1.05
        )
    ...
    if show_all:
        plt.savefig(
            osp.join(
                save_path,
                f"{transform_type.lower()}_equivariance_test_internal_{irrep_str}_all.pdf",
            ),
            bbox_inches="tight",
            transparent=True,
        )
        ...
        plt.savefig(
            osp.join(
                save_path,
                f"{transform_type.lower()}_equivariance_test_internal_{irrep_str}.pdf",
            ),
            bbox_inches="tight",
            transparent=True,
        )
...
def plot_output_dev(dev_output, alphas, transform_type, weight, save_path):
    ...
    if transform_type.lower() in ["boost", "boosts"]:
        if weight == (1, 1):
            title = rf"Boost equivariance test of reconstructed {irrep_str} $p^\mu$"
        ...
        if weight == (1, 1):
            title = rf"Rotation equivariance test of reconstructed {irrep_str} $p^\mu$"
    ...
    plt.savefig(
        osp.join(
            save_path,
            f"{transform_type.lower()}_equivariance_test_reconstructed_{irrep_str}.pdf",
        ),
        bbox_inches="tight",
        transparent=True,
    )
...
def plot_all_dev(dev, save_path):
    ...
    pt_path = make_dir(osp.join(save_path, "pt_files"))
    torch.save(
        dev["perm_invariance_dev_output"],
        osp.join(pt_path, "perm_invariance_dev_output.pt"),
    )
    torch.save(
        dev["perm_equivariance_dev_output"],
        osp.join(pt_path, "perm_equivariance_dev_output.pt"),
    )

    for weight in [(0, 0), (1, 1)]:
        plot_output_dev(
            dev_output=dev["boost_dev_output"],
            alphas=dev["gammas"],
        ...
        for show_option in [True, False]:
            plot_internal_dev(
                dev_internal=dev["boost_dev_internal"].copy(),
                alphas=dev["gammas"],
                transform_type="boost",
                weight=weight,
                save_path=save_path,
                show_all=show_option,
            )
            plot_internal_dev(
                dev_internal=dev["rot_dev_internal"].copy(),
                alphas=dev["thetas"],
                transform_type="rotation",
                weight=weight,
                save_path=save_path,
