...
from lgn.models.autotest.utils import (
    get_output,
    get_dev,
    get_avg_output_dev,
    get_avg_internal_dev,
    get_node_dev,
    display_err,
)
...
def _gen_rot(
    ...
    R = torch.stack(
        (
            D[(1, 1)][0].matmul(cartesian4[0]) - D[(1, 1)][1].matmul(cartesian4[1]),
...
def covariance_test(
    ...
    data = data.copy()
    if unit.lower() == "gev":
        # Convert to TeV for better numerical precision after boost
        data["p4"] /= 1e3
        # # Convert to PeV for better numerical precision after boost
        # data['p4'] /= 1e6
    # elif unit.lower() == 'tev':
    #     # Convert to PeV for better numerical precision after boost
    #     data['p4'] /= 1e3
    ...
    if test_type.lower() in ["boost", "boosts"]:
        if alpha_max is None:
            alpha_max = 10.0
        alpha_range = np.arange(0, alpha_max + 0.01, step=alpha_max / 25.0)
        gammas, boost_dev_output, boost_dev_internal = boost_equivariance(
            encoder, decoder, data, alpha_range, axis, device, dtype, cg_dict
        )
        ...
        if alpha_max is None:
            alpha_max = 2 * np.pi
        theta_range = np.arange(0, alpha_max + 0.01, step=alpha_max / 25.0)
        theta_range, rot_dev_output, rot_dev_internal = rot_equivariance(
            encoder, decoder, data, theta_range, axis, device, dtype, cg_dict
        )
...
def permutation_invariance_test(encoder, decoder, data, *ignore):
    ...
    data_perm = {
        key: apply_perm(val) if key in ["p4", "scalars"] else val
    ...
    perm_outputs = {
        k: torch.stack((apply_perm(v[0]), apply_perm(v[1])), dim=0)
        for k, v in outputs_noperm.items()
    }

    perm_inv_dev = get_node_dev(outputs_perm, outputs_noperm, mode="max")
    perm_equivariance_dev = get_node_dev(outputs_perm, perm_outputs, mode="max")

    return perm_inv_dev, perm_equivariance_dev


def boost_equivariance(
    encoder, decoder, data, alpha_range, axis, device, dtype, cg_dict
):
    ...
    for alpha in alpha_range:
        ...
        boost_internal = [
            rot.rotate_rep(internal[i], phi_x, phi_y, phi_z, cg_dict=cg_dict)
...
def rot_equivariance(encoder, decoder, data, theta_range, axis, device, dtype, cg_dict):
    ...
    for theta in theta_range:
        ...
        rot_internal = [
            rot.rotate_rep(internal[i], theta_x, theta_y, theta_z, cg_dict=cg_dict)
            for i in range(len(internal))
        ]
...
def lgn_tests(
    ...
    for idx, data in enumerate(tqdm(dataloader)):
        ...
        perm_inv, perm_equivariance = permutation_invariance_test(
    ...
    lgn_test_results["boost_dev_output"] = get_avg_output_dev(
        boost_test_all_epochs, "boost"
    )
    lgn_test_results["boost_dev_internal"] = get_avg_internal_dev(
        boost_test_all_epochs, "boost"
    )

    lgn_test_results["thetas"] = rot_test_all_epochs[0]["thetas"]
    lgn_test_results["rot_dev_output"] = get_avg_output_dev(
        rot_test_all_epochs, "rotation"
    )
    lgn_test_results["rot_dev_internal"] = get_avg_internal_dev(
        rot_test_all_epochs, "rotation"
    )

    perm_inv_test_avg = {
        key: sum(dev[key] for dev in perm_inv_test_all_epochs)
        / len(perm_inv_test_all_epochs)
        for key in perm_inv_test_all_epochs[0].keys()
    }
    perm_equivariance_test_avg = {
        key: sum(dev[key] for dev in perm_equivariance_test_all_epochs)
        / len(perm_equivariance_test_all_epochs)
    ...
    display_err(
        lgn_test_results["gammas"],
        lgn_test_results["boost_dev_output"],
        alpha_name="gamma",
        caption="Output relative error",
    )
    ...
    display_err(
        lgn_test_results["thetas"],
        lgn_test_results["rot_dev_output"],
        alpha_name="theta",
        caption="Output relative error",
    )
    ...
    print(
        f"Permutation invariance test result: {lgn_test_results['perm_invariance_dev_output']}"
    )
    print(
        f"Permutation equivariance test result: {lgn_test_results['perm_equivariance_dev_output']}"
    )
    print(SEPARATOR)
