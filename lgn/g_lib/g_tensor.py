...
class GTensor(ABC):
    ...
    def truncate(self, maxdim):
        ...
        `GTensor` if they correspond to weights greater than `maxdim`.
    ...
    def bshape(self):
        """
        Get a list of shapes of each `torch.Tensor`
        """
    ...
    def channels(self):
        """
        Constructs `GTau`, and then gets the corresponding `GTau.channels`
        method.
        """
    ...
    def abs(self):
        """
        Calculate the element-wise absolute value of the `torch.GTensor`.
    ...
    def rand(cls, batch, tau, device=None, dtype=None, requires_grad=False, real=False):
        """
        Factory method to create a new random `GVec`.
        """
