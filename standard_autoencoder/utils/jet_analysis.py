...
def plot_p(args, p_target, p, save_dir,
    ...
    # tuples
    p_target_polar = get_p_polar(p_target, cutoff=cutoff, eps=EPS)
    jet_target_polar = get_jet_feature_polar(p_target)
    p_gen_polar = get_p_polar(p, cutoff=cutoff, eps=EPS)
    jet_gen_polar = get_jet_feature_polar(p)

    p_target_cartesian = get_p_cartesian(p_target.detach().cpu().numpy(), cutoff=cutoff)
    jet_target_cartesian = get_jet_feature_cartesian(p_target.detach().cpu().numpy())
    p_gen_cartesian = get_p_cartesian(p.detach().cpu().numpy(), cutoff=cutoff)
    jet_gen_cartesian = get_jet_feature_cartesian(p.detach().cpu().numpy())

    # arrays
    jets_target = get_p_polar_tensor(p_target, eps=EPS)
    jets_gen = get_p_polar_tensor(p, eps=EPS)
...
def plot_jet_image(args, p_target, p, save_dir, epoch, vmin=1e-10, show=False):
    ...
    target_pix = get_average_jet_image(p_target, npix=args.jet_image_npix)
    gen_pix = get_average_jet_image(p, npix=args.jet_image_npix)
