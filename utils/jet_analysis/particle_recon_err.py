...
def get_rel_err_find_match(
    ...
    cost_cartesian = (
        torch.cdist(p_target_cartesian, p_recons_cartesian).cpu().detach().numpy()
    )
    cost_polarrel = (
        torch.cdist(p_target_polarrel, p_recons_polarrel).cpu().detach().numpy()
    )
    ...
    for i in range(len(p_target_cartesian)):
        ...
        rel_err_polar = (p_recons_polar[i][matching[1]] - p_target_polar[i]) / (
            p_target_polar[i] + EPS
        )
        ...
        rel_err_polarrel = (
            p_recons_polarrel[i][matching[1]] - p_target_polarrel[i]
        ) / (p_target_polarrel[i] + EPS)
