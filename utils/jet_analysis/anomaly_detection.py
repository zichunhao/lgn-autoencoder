...
CHAMFER_PARTICLE_NORMALIZED_CARTESIAN = (
    "particle, normalized Cartesian, Chamfer distance"
)
...
HUNGARIAN_PARTICLE_NORMALIZED_CARTESIAN = (
    "particle, normalized Cartesian, Hungarian distance"
)
...
def plot_roc_curves(
    ...
    for kind, auc in aucs:
        ...
        for y_value in rocs_hlines:
            ...
            intercepts[y_value] = x_intercept
            ax.vlines(
                x=x_intercept,
                ymin=0,
                ymax=y_value,
...
def anomaly_scores(
    ...
    scores = {
        ...
        .detach()
        .numpy(),
        CHAMFER_PARTICLE_NORMALIZED_CARTESIAN: chamfer(
            recons_normalized, target_normalized, batch_size=batch_size
        )
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
        CHAMFER_PARTICLE_NORMALIZED_POLAR: chamfer(
            recons_normalized_polar, target_normalized_polar, batch_size=batch_size
        )
        .mean(-1)
        .cpu()
        .detach()
        ...
        HUNGARIAN_PARTICLE_CARTESIAN: hungarian(recons, target, batch_size=batch_size)
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
        ...
        .detach()
        .numpy(),
        HUNGARIAN_PARTICLE_NORMALIZED_CARTESIAN: hungarian(
            recons_normalized, target_normalized, batch_size=batch_size
        )
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
        HUNGARIAN_PARTICLE_NORMALIZED_POLAR: hungarian(
            recons_normalized_polar, target_normalized_polar, batch_size=batch_size
        )
        .mean(-1)
        .cpu()
        ...
        .detach()
        .numpy(),
        MSE_PARTICLE_NORMALIZED_CARTESIAN: mse(recons_normalized, target_normalized)
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
        MSE_PARTICLE_NORMALIZED_POLAR: mse(
            recons_normalized_polar, target_normalized_polar
        )
        .mean(-1)
        .cpu()
        .detach()
        ...
        CHAMFER_PARTICLE_LORENTZ: chamfer_lorentz(recons, target, batch_size=batch_size)
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
        HUNGARIAN_PARTICLE_LORENTZ: hungarian_lorentz(
        ...
        .detach()
        .numpy(),
        MSE_PARTICLE_LORENTZ: mse_lorentz(recons, target)
        .mean(-1)
        .cpu()
        .detach()
        .numpy(),
