...
def train_loop(
    ...
    for ep in range(args.num_epochs):
        ...
        if abs(valid_avg_loss) < best_loss:
            ...
            torch.save(
                {"best_epoch": epoch, "best_loss": best_loss},
        ...
        if to_plot:
            for target, recons, dir in zip(
                ...
                plot_p(
                    args, target, recons, save_dir=dir, cutoff=args.cutoff, epoch=epoch
                )
        ...
        np.savetxt(
            osp.join(outpath, "model_evaluations/losses_training.txt"), train_avg_losses
        )
        np.savetxt(
            osp.join(outpath, "model_evaluations/losses_validation.txt"),
            valid_avg_losses,
        )
        ...
        if (epoch > 0) and ((epoch + 1) % 10 == 0):
            plot_eval_results(
                args,
                data=(train_avg_losses[-10:], valid_avg_losses[-10:]),
...
def get_loss(
    ...
    if "chamfer" in loss_choice:
        ...
        batch_loss = chamferloss(
            p4_recons, p4_target, jet_features=args.chamfer_jet_features
        )  # output, target
