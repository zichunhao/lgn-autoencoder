...
def normalize_p4(
    p4: torch.Tensor, method: str = "overall_max"
    ...
    if method.lower().replace(" ", "_").replace("-", "_") == "component_max":
        ...
        logging.warning(
            f"Normalization method {method} not recognized. Using component_max."
        )
        return normalize_p4_overall_max(p4)
...
def normalize_p4_overall_max(p4: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    normalize_p4_factor = (
        torch.abs(p4).amax(dim=-1, keepdim=True).amax(dim=-2, keepdim=True) + EPS
    )
    p4 = p4 / normalize_p4_factor
    return p4, normalize_p4_factor
